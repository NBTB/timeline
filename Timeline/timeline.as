package Timeline
{
	...
	{
		var timelineLine:MovieClip;
		var timelineEventList:Vector.<TimelineEvent>;		//stores all the events from our database in sorted order
		var currentEventList:Vector.<TimelineEvent>;		//stores the currently shown events
		var iconArray:Vector.<Bitmap>;						//stores all the icons
		var zoomLevel:int;			
		var MAXZOOM:int = 5;
		var MINZOOM:int = 0;
		...
		var beginDate:int;
		var endDate:int;
		var MINDATE:int = 1810;
		var MAXDATE:int = 1895;
		var lineStart:int = 100;
		var lineEnd:int = 650;
		var lineHeight:int = 500;
		...
		public function changeZoomLevel(beginDateZoom:int, endDateZoom:int)
		{			
		...
		private function populateTimeline()
		{			
		...
		private function emptyTimeline()
		{
			...
			for(var i=0; i<timelineEventList.length; i++)
			{
				if(timelineEventList[i].year >= beginDate && timelineEventList[i].year < endDate)
				{
					...
					else
					{
						...
						var bit:Bitmap = new Bitmap((iconArray[0]).bitmapData.clone());
						bit.scaleX = 0.1;
						bit.scaleY = 0.1;
						bit.x = circle.x - bit.width/2;
